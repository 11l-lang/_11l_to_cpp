...
def parse_internal(this_node):
    ...
    def new_scope(node, func_args = None):
        next_token()
        assert(token.category == Token.Category.SCOPE_BEGIN) # [-error message ‘expected an indented block’ is already generated by tokenizer, so there is just an assert-]
        next_token()
        parse_internal(node)
